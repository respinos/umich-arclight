...
to_field 'places_sim', extract_xpath('/ead/archdesc/controlaccess/(controlaccess|.)/geogname')
to_field 'places_ssim', extract_xpath('/ead/archdesc/controlaccess/(controlaccess|.)/geogname')
to_field 'places_ssm', extract_xpath('/ead/archdesc/controlaccess/(controlaccess|.)/geogname')
...
to_field 'access_subjects_ssim', extract_xpath('/ead/archdesc/controlaccess/(controlaccess|.)', to_text: false) do |_record, accumulator|
...
to_field 'formats_ssim', extract_xpath('/ead/archdesc/controlaccess/(controlaccess|.)/genreform')
...
NAME_ELEMENTS.map do |selector|
  to_field 'names_coll_ssim', extract_xpath("/ead/archdesc/controlaccess/(controlaccess|.)/#{selector}"), unique, strip
...
compose 'components', ->(record, accumulator, _context) { accumulator.concat record.xpath('//*[is_component(.)]', NokogiriXpathExtensions.new) } do
  ...
  NAME_ELEMENTS.map do |selector|
    to_field 'names_ssim', extract_xpath("./controlaccess/(controlaccess|.)/#{selector}"), unique, strip
    to_field "#{selector}_ssm", extract_xpath(".//#{selector}"), unique, strip
  end

  # DUL CUSTOMIZATION: Bugfix field geogname --> places
  to_field 'places_sim', extract_xpath('./controlaccess/(controlaccess|.)/geogname')
  to_field 'places_ssm', extract_xpath('./controlaccess/(controlaccess|.)/geogname')
  to_field 'places_ssim', extract_xpath('./controlaccess/(controlaccess|.)/geogname')

  to_field 'access_subjects_ssim', extract_xpath('./controlaccess/(controlaccess|.)', to_text: false) do |_record, accumulator|
    accumulator.map! do |element|
      # DUL CUSTOMIZATION: pull out genreform into its own field
      %w[subject function occupation].map do |selector|
        element.xpath(".//#{selector}").map(&:text)
  ...
  # DUL CUSTOMIZATION: capture formats (genreform) field separately from subjects
  to_field 'formats_ssim', extract_xpath('./controlaccess/(controlaccess|.)/genreform')
  to_field 'formats_ssm' do |_record, accumulator, context|
